...
<?php  
  if (isset($_POST['submit'])) {
    $target = "book/bookleveltow/".basename($_FILES['book']['name']);
    $namebook = mysqli_escape_string($conn, $_POST['namebook']);
    $levelid = mysqli_escape_string($conn, $_POST['levelid']);
    $image = $_FILES['book']['name'];
    $sql = mysqli_query($conn, "INSERT INTO bookleveltow (namebook, book, levelid)VALUES ('$namebook', '$book', '$levelid')");
    move_uploaded_file($_FILES['book']['tmp_name'], $target);

    if ($sql) {
      echo "<script> alert('New bookleveltow added...!!!') </script>";
      echo "<script> location = 'bookleveltow.php' </script>";
    }
 }
?>
        ...
        <div class="col-md-12">
            <div class="card">
                <form action="" method="post" class="form-horizontal" enctype="multipart/form-data">
                    <div class="card-body">
                        <h4 class="card-title">كتب المستوى الثاني</h4>
                        ...
                        <div class="form-group row">
                            <label for="levelid" class="col-sm-3 text-right control-label col-form-label"> المستوى الثاني</label>
                            <div class="col-sm-9">
                                    ...
                                    <div class="card mb-4">
                                      ...
                                      <div class="table-responsive p-3">
                                          <table class="table align-items-center table-flush table-hover" id="dataTableHover">
                                                ...
                                                $query12 = "SELECT bookleveltow.id,level.levelname,level.section,level.id 
                                                as id,bookleveltow.id,bookleveltow.levelid,bookleveltow.namebook,bookleveltow.id
                                                FROM bookleveltow
                                                INNER JOIN level ON level.id = bookleveltow.levelid";
